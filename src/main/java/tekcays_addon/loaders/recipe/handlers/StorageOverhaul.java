...
public class StorageOverhaul {
    ...
    public static void shapesRecipesTanksAndValves() {
        // Tanks
        ModHandler.addShapedRecipe(true, "wood_tank", MODULABLE_TANK[0].getStackForm(),
                " R ", "rCs", " R ",
                'R', new UnificationEntry(OrePrefix.ring, Materials.Lead),
                'C', MetaBlocks.STEAM_CASING.getItemVariant(WOOD_WALL));

        ModHandler.addShapedRecipe(true, "steel_tank", MODULABLE_TANK[1].getStackForm(),
                " R ", "hCw", " R ",
                'R', new UnificationEntry(OrePrefix.ring, Materials.Steel),
                'C', TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.STEEL_WALL));

        ModHandler.addShapedRecipe(true, "stainless_steel_tank", MODULABLE_TANK[3].getStackForm(),
                " R ", "hCw", " R ",
                'R', new UnificationEntry(OrePrefix.ring, Materials.StainlessSteel),
                'C', TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.STAINLESS_STEEL_WALL));

        ModHandler.addShapedRecipe(true, "galvanized_steel_tank", MODULABLE_TANK[2].getStackForm(),
                " R ", "hCw", " R ",
                'R', new UnificationEntry(OrePrefix.ring, TKCYAMaterials.GalvanizedSteel),
                'C', TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.GALVANIZED_STEEL_WALL));
        ...
        ModHandler.addShapedRecipe(true, "tank_valve.steel", TKCYAMetaTileEntities.STEEL_TANK_VALVE.getStackForm(),
                ...
                'R', new UnificationEntry(OrePrefix.ring, Materials.Steel), 'C', TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.STEEL_WALL));
        ...
        ModHandler.addShapedRecipe(true, "wall.steel",
                TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.STEEL_WALL),
                "hdw", "PP ", "PP ",
    ...
    public static void drumsAssembler() {
        ...
        for (int i = 1; i < DRUM_MATERIALS.size(); i++) {
            ...
            ASSEMBLER_RECIPES.recipeBuilder().EUt(16).input(stickLong, m, 2).input(curvedPlate, m, 4)
                    .outputs(DRUMS[i].getStackForm()).duration(200).circuitMeta(2).buildAndRegister();
        }
    }

    public static void wallsAssembler() {
        ASSEMBLER_RECIPES.recipeBuilder().EUt(16).input(plate, Steel, 4)
                .outputs(TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.STEEL_WALL))
                .duration(200).circuitMeta(8).buildAndRegister();
        ASSEMBLER_RECIPES.recipeBuilder().EUt(16).input(plate, StainlessSteel, 4)
                .outputs(TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.STAINLESS_STEEL_WALL))
                .duration(200).circuitMeta(8).buildAndRegister();
        ASSEMBLER_RECIPES.recipeBuilder().EUt(16).input(plate, GalvanizedSteel, 4)
                .outputs(TKCYAMetaBlocks.LARGE_MULTIBLOCK_CASING
                        .getItemVariant(BlockLargeMultiblockCasing.CasingType.GALVANIZED_STEEL_WALL))
                .duration(200).circuitMeta(8).buildAndRegister();
    }
