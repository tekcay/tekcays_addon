...
public class CokeOvenRecipeHandler {
    ...
    public static void init() {
        COKING.recipeBuilder().input(log, Wood).output(gem, Charcoal).fluidOutputs(Creosote.getFluid(250))
                .duration(5 * MINUTE).buildAndRegister();
        COKING.recipeBuilder().input(gem, Coal).output(gem, Coke).fluidOutputs(Creosote.getFluid(500))
                .duration(4 * MINUTE).buildAndRegister();
        COKING.recipeBuilder().input(block, Coal).output(block, Coke).fluidOutputs(Creosote.getFluid(4500))
                .duration(45 * MINUTE).buildAndRegister();
