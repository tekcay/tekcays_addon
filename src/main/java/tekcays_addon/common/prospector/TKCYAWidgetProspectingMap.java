package tekcays_addon.common.prospector;
...
public class TKCYAWidgetProspectingMap extends Widget {
    ...
    public TKCYAWidgetProspectingMap(int xPosition, int yPosition, int chunkRadius, WidgetOreList widgetOreList, int mode, int scanTick) {
    ...
    @SideOnly(Side.CLIENT)
    public TKCYAWidgetProspectingMap setOnPacketReceived(Consumer<PacketProspecting> onPacketReceived) {
        ...
        if (FMLCommonHandler.instance().getMinecraftServerInstance().getTickCounter() % scanTick == 0 && chunkIndex < (chunkRadius * 2 - 1) * (chunkRadius * 2 - 1)) {
            ...
            switch (mode) {
                ...
                case FLUID_PROSPECTING_MODE:
                    FluidDepositWorldEntry fStack = FluidDepositHandler.getFluidVeinWorldEntry(world, chunk.x, chunk.z);
                    if (fStack != null && fStack.getFluidDepositDefinition() != null) {
                        packet.addBlock(0, 3, 0, String.valueOf(FluidDepositHandler.getFluidAmount(world, chunk.x, chunk.z)));
                        packet.addBlock(0, 2, 0, String.valueOf(FluidDepositHandler.getDepth(world, chunk.x, chunk.z)));
                        packet.addBlock(0, 1, 0, FluidDepositHandler.getFluidInChunk(world, chunk.x, chunk.z).getName());
                    }
                    break;
                default:
                    break;
