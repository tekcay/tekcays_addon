...
import tekcays_addon.common.prospector.TKCYAWidgetProspectingMap;
...
public class TKCYAProspectorScannerBehavior implements IItemBehaviour, ItemUIFactory, SearchComponent.IWidgetSearch<String> {
    ...
    public TKCYAProspectorScannerBehavior(int radius, int tier) {
    ...
    public ActionResult<ItemStack> onItemRightClick(@Nonnull World world, @Nonnull EntityPlayer player, EnumHand hand) {
        ...
        if (!world.isRemote) {
            if (player.isSneaking()) {
                ...
                if (nextMode == TKCYAWidgetProspectingMap.FLUID_PROSPECTING_MODE) {
    ...
    private static int getNextMode(int mode) {
        return mode == TKCYAWidgetProspectingMap.ORE_PROSPECTING_MODE ? 1 : 0;
    }
    ...
    public ModularUI createUI(PlayerInventoryHolder holder, @Nonnull EntityPlayer entityPlayer) {
        ...
        builder.widget(new TKCYAWidgetProspectingMap(6, 18, radius, this.widgetOreList, mode, 1));
    ...
    public void addInformation(ItemStack itemStack, List<String> lines) {
        ...
        if (tier >= FLUID_PROSPECTION_THRESHOLD) {
            ...
            lines.add(I18n.format(getMode(itemStack) == TKCYAWidgetProspectingMap.ORE_PROSPECTING_MODE ? "metaitem.prospector.mode.ores" : "metaitem.prospector.mode.fluid"));
        } else {
