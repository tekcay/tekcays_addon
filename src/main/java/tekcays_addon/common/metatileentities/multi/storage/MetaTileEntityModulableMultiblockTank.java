...
public class MetaTileEntityModulableMultiblockTank extends MultiblockWithDisplayBase {
    ...
    public String getFillPercentage() {
        return isTankEmpty() ? "0% Filled" : BigDecimal.valueOf(100 * this.importFluids.getTankAt(0).getFluidAmount())
                .divide(BigDecimal.valueOf(this.actualCapacity), 1, BigDecimal.ROUND_UP).toString() + "% Filled";
    }

    public String getTankContent() {
        return isTankEmpty() ? "Empty" : this.importFluids.getTankAt(0).getFluidAmount() / 1000 + " kL of " +
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        if (!this.isStructureFormed()) {
            ...
            textList.add(new TextComponentTranslation("tkcya.multiblock.modulable_tank.capacity",
    ...
    public void addInformation(ItemStack stack, @Nullable World player, @NotNull List<String> tooltip,
        ...
        if (material.equals(TreatedWood)) {
            ...
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
                tooltip.add(
                        I18n.format("gregtech.fluid_pipe.max_temperature", pipeProperties.getMaxFluidTemperature()));
