...
public class MetaTileEntityLaserEjector extends MetaTileEntity implements IDataInfoProvider, IActiveOutputSide {

    private static final int PRESSURE_DECREASE = -1000;
    private static final int STEAM_CONSUMPTION = 160;

    private LaserContainer pressureContainer;
    private FluidTank fuelFluidTank;
    @SuppressWarnings("FieldMayBeFinal")
    private boolean isHighLaser;

    protected EnumFacing outputFacing;

    public MetaTileEntityLaserEjector(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId);
        this.isHighLaser = true;
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity metaTileEntityHolder) {
        return new MetaTileEntityLaserEjector(metaTileEntityId);
    }

    @Override
    protected void initializeInventory() {
        super.initializeInventory();
        this.pressureContainer = new LaserContainer(this, 13E-5, GCYSValues.EARTH_PRESSURE, 1.0);
    }
    ...
    protected ModularUI createUI(@Nonnull EntityPlayer entityPlayer) {
        return ModularUI.builder(GuiTextures.BACKGROUND_STEAM.get(isHighLaser), 176, 166)
                ...
                .widget(new TankWidget(fuelFluidTank, 70, 26, 10, 54)
                        .setBackgroundTexture(GuiTextures.PROGRESS_BAR_BOILER_EMPTY.get(isHighLaser)))

                .bindPlayerInventory(entityPlayer.inventory, GuiTextures.SLOT_STEAM.get(isHighLaser), 0)
    ...
    public void update() {
        ...
        if (!getWorld().isRemote && getOffsetTimer() % 20 == 0) {
            if (pressureContainer.getLaser() > pressureContainer.getMinLaser()) {
                ...
                if (drained != null && drained.amount == STEAM_CONSUMPTION && ventSteam(true)) {
                    ...
                    if (pressureContainer.changeParticles(PRESSURE_DECREASE, true)) {
                        ...
                        pressureContainer.setParticles(pressureContainer.getMinLaser() * pressureContainer.getVolume());
                    }
            ...
            if (te != null) {
                ILaserContainer container = te.getCapability(TKCYATileCapabilities.CAPABILITY_LASER_CONTAINER, this.getOutputFacing().getOpposite());
                if (container != null) {
